...
def md_to_manifest(markdown_folder, manifest_folder, mail, password) :
    ...
    titre_url = titre.replace(" ","_")
    config.configs['helpers.auto_fields.AutoLang'].auto_lang = "en"
    base_url = "http://127.0.0.1:5500"
    manifest = Manifest(id=f"{base_url}/{titre_url}.json", label=titre)
    ...
    for i in link:
            ...
            if format[num] == "video"  :
                ...
                anno_page = AnnotationPage(id=f"{base_url}/canvas/{num}/page/1")
                anno = Annotation(id=f"{base_url}/canvas/{num}/page/1/1",
                            motivation="painting",
                            body=anno_body,
                            target=canvas.id)
            ...
            if format[num] == "image"  :
                ...
                anno_page = canvas.add_image(image_url = i,
                                                anno_page_id=f"{base_url}/canvas/{num}/page/1",
                                                anno_id=f"{base_url}/canvas/{num}/page/1/1",
...
def metadata_update(markdown_folder, manifest_folder, mail, password):
    ...
    chemin = os.path.join(markdown_folder)
...
def md_to_anotation(markdown_folder, manifest_folder, mail, password) :
      ...
      manifest = Manifest(id=f"{base_url}",
                    label=titre)
